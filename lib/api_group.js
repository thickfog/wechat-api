 ...
 * Examples:
 * ```
 * api.moveUsersToGroup(openidList, groupId, callback);
 * ```
 * Callback:
 *
 * - `err`, 调用失败时得到的异常
 ...
 * {"errcode": 0, "errmsg": "ok"}
 * ```
 * @param {array} openidList 用户的openid
 * @param {Number} groupId 分组ID
 * @param {Function} callback 回调函数
 */
exports.moveUsersToGroup = function (openidList, groupId, callback) {
  this.preRequest(this._moveUsersToGroup, arguments);
};

/*!
 * 移动多用户进分组的未封装版本
 */
exports._moveUsersToGroup = function (openidList, groupId, callback) {
  // http请求方式: POST（请使用https协议）
  // https://api.weixin.qq.com/cgi-bin/groups/members/batchupdate?access_token=ACCESS_TOKEN
  // POST数据格式：json
  // POST数据例子：{"openidList":["oDF3iYx0ro3_7jD4HFRDfrjdCM58"],"to_groupid":108}
  var url = this.prefix + 'groups/members/batchupdate?access_token=' + this.token.accessToken;
  var data = {
